...
def do(video_id, session, proxies=None):
    try:
        ...
        if r'"playabilityStatus":{"status":"ERROR"' in res.text or r'"playabilityStatus":{"status":"LOGIN_REQUIRED","messages":["This is a private video.' in res.text \
        ...
        if r'Sorry for the interruption.' in res.text:
            print('notwork vloume error')
            conn.commit()
            conn.close()
            return False

        view = res.text.split(r'{"videoViewCountRenderer":{"viewCount":{"simpleText":"')[1].split(' ')[0]
        ...
        if len(tags) >= 1:
            sql += " ".join(["INSERT INTO video_keyword_new (video_idx, keyword) VALUES", "".join(tags)[:-1],
